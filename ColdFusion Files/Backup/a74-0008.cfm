...
<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
	...
	<cfquery name="getComms" datasource="#dsn#">
		...
		WHERE C.TermYear = <cfqueryparam cfsqltype="cf_sql_smallint" value="#year#">
			...
			AND C.Reference IN ('B','C','D') AND C.Type = 1 AND C.Status = 1 AND S.Comment <> ''
		ORDER BY C.Seq
	</cfquery>
	<cfquery name="getSubj" datasource="#dsn#">
		...
		<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
			...
			<cfif getComments3.RecordCount EQ 0 AND getComments4.RecordCount EQ 0>
				<cfloop query="getComms">
					...
					<tr><td class="ball txtRep11Bold" style="padding:1px 5px;">#HeadingRep# <div style="float:right;padding-right:50px;" class="txtRep11Bold"><cfif Find('Head',HeadingRep)>#getCF.Title# #LEFT(getCF.FirstName, 1)# #getCF.LastName#<cfelseif Find('Class',HeadingRep) OR Find('Learning', HeadingRep)>#getList.Title[i]# #LEFT(getList.TFName[i], 1)# #getList.TLName[i]#</cfif></div></td></tr>
					<tr><td class="bleft bbottom bright txtRep10" style="padding:1px 5px;height:35px;vertical-align:top;line-height:11.5pt;">#Replace(Comment, Chr(13), "<br />", "All")#</td></tr>
				</cfloop>
			</cfif>
		</table>
