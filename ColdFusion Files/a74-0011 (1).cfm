...
<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
			...
			<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
				...
				<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
					<tr>
						...
						<td align="right" rowspan="2"><img src="#fileService.getFile(fileDir = "/keydoc/stphotos", filename = "#getList.Photo[i]#", includeHTTP = "https:")#" style="height:85px;border:3px solid darkgreen;padding:1px;"></td>
					</tr>
					<tr>
				...
				<tr style="background-color:##d3d3d3;">
					...
					<td align="right" style="padding:3px 5px;" class="txtRep12Bold btop bbottom bright">Year: #Replace(Replace(ListFirst(getList.AcYear[i],'_'),'Yr1','One'),'Yr2','Two')#</td>
				</tr>
				<tr>
	...
	<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
		<tr>
			<td class="btop bleft bbottom txtRep11" align="left" style="padding:1px 3px;width:33%;"><b>Age:</b> #Replace(Replace(getList.Age[i],'-0','y '),'-','y ')#m</td>
			<td class="btop bbottom txtRep11" align="center" style="padding:1px 3px;width:33%;"><b>Average age of class:</b> #AvgYR#y #AvgMT#m</td>
			<td class="btop bbottom bright txtRep11" align="right" style="padding:1px 3px;width:33%;"><b>Days Absent:</b> #NumberFormat(getStudAtt.Absent)#</td>
		</tr>
		<tr><td colspan="3" class="txtRep11 bleft bbottom bright" style="padding:2px 3px;line-height:12pt;">#getCF.RepData#</td></tr>
	</table>
	</td>
	</tr>
	...
	<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
		...
		<cfloop query="getGradeSch">
			<tr>
				<td class="bleft bbottom bright txtRep11Bold" align="center" style="width:10%;">#Grade#</td>
				<td class="bbottom bright txtRep10" align="left" style="width:90%;padding:1px 5px;line-height:11pt;">#GradeDesc#</td>
			</tr>
		</cfloop>
	</table>
	</td>
	...
	<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
		...
		<cfloop query="getOpt2">
			<tr>
				<td class="bleft bbottom bright txtRep11Bold" align="left" style="width:30%;padding:1px 5px;">#OptName#</td>
				<td class="bbottom bright txtRep11" align="left" style="width:70%;padding:1px 5px;">#OptDescript#</td>
			</tr>
		</cfloop>
	</table>
	</td>
	...
	<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
		<cfif getSubj.RecordCount GT 0>
			...
			<cfloop query="getSubj">
				<tr>
					<td class="bleft bbottom bright txtRep11Bold" style="padding:1px 4px 2px 4px;;width:30%;line-height:11.5pt;">#SubjName#</td>
					<td class="bbottom bright txtRep11" align="center" style="padding:2px 4px;width:15%;">#Grade#</td>
					<td class="bbottom bright txtRep11" align="center" style="padding:2px 0px;width:20%;">#Effort#</td>
					<td class="bbottom bright txtRep10" align="left" style="padding:1px 4px;width:35%;">#T1Title#. #LEFT(T1FName, 1)# #T1LName#</td>
				</tr>
			</cfloop>
		</cfif>
	</table>
...
<cfloop query="getPage">
<cfquery name="getSubj" datasource="#dsn#">
	SELECT A.SCode, A.SubjName, A.T1Title, A.T1FName, A.T1LName, SS.Comment
	FROM vARSubjAcads A
		LEFT JOIN ARCommSetup CS ON CS.TermYear = A.TermYear AND CS.Term = A.Term AND CS.StudYear = A.AcYear AND CS.Type = 2 AND CS.Reference = 'A'
		LEFT JOIN ARAcadComm SS ON SS.CommSetupID = CS.CommSetupID AND SS.AcadID = A.AcadID
	WHERE A.TermYear = #year# AND A.Term = #term# AND A.StudID = #RecNo# AND A.RepCard = 0 AND A.SType <> 'R' AND A.Page = #Page# AND SS.Comment <> ''
	ORDER BY A.Seq
</cfquery>

<cfquery name="getComms" datasource="#dsn#">
	SELECT C.HeadingRep, S.Comment, C.TextType, C.Reference
	FROM ARCommSetup C
	...
	WHERE C.TermYear = <cfqueryparam cfsqltype="cf_sql_smallint" value="#year#">
		...
		AND C.Reference IN ('B','C','D') AND C.Type = 1 AND C.Status = 1
	ORDER BY C.Seq
</cfquery>
...
<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
	...
	<table cellpadding="0" cellspacing="0" align="center" style="width:100%;">
		<cfloop query="getSubj">
			<tr><td class="ball txtRep11Bold" style="padding:1px 5px;background-color:##D9D9D9;">#SubjName# <div style="float:right;padding-right:50px;" class="txtRep11Bold">#T1Title#. #LEFT(T1FName, 1)# #T1LName#</div></td></tr>
			<tr><td class="bleft bbottom bright txtRep11" style="padding:1px 5px;height:35px;vertical-align:top;">#Replace(Comment, Chr(13), "<br />", "All")#</td></tr>
			<tr><td style="height:10px;"></td></tr>
		</cfloop>
		<cfif getPage.CurrentRow EQ getPage.RecordCount>
			<cfloop query="getComms">
				<cfif getComms.Comment NEQ "">
				<cfif CurrentRow GT 1><tr><td style="height:10px;"></td></tr></cfif>
				<tr><td class="ball txtRep11Bold" style="padding:1px 5px;background-color:##D9D9D9;">#HeadingRep# <div style="float:right;padding-right:50px;" class="txtRep11Bold"><cfif Find('Head',HeadingRep)>#getCF.Title# #LEFT(getCF.FirstName, 1)# #getCF.LastName#<cfelse>#getList.Title[i]# #LEFT(getList.TFName[i], 1)# #getList.TLName[i]#</cfif></div></td></tr>
				<tr><td class="bleft bbottom bright txtRep11" style="padding:1px 5px;height:35px;vertical-align:top;">#Replace(Comment, Chr(13), "<br />", "All")#</td></tr>
			    </cfif>
			</cfloop>
		</cfif>
