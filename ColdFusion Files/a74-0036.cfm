...
<cfquery name="getAverage" datasource="#dsn#">
	SELECT SUM(Round(A.Mark, 0)) AS TotalMark, Avg(Round(A.Mark, 0)) AS AvgMark
    FROM (
    SELECT A.SCode, A.SSID, ROUND(D2.Mark, 2) AS Mark, A.SubjWeight, IsNull(C.IsCore, 1) AS IsCore,
    C.TopNum, ROW_NUMBER() OVER (PARTITION BY C.IsCore ORDER BY C.IsCore DESC, ROUND(D2.Mark,2)DESC) AS RowNo
    FROM vARSubjAcads A
	...
	WHERE (IsCore = 1 AND (RowNo <= TopNum OR TopNum IS NULL)) OR (IsCore = 0 AND (RowNo <= TopNum OR TopNum IS NULL))
</cfquery>
...
<cfquery name="getMerit" datasource="#dsn#">
    SELECT SUM(L.Points) AS Points
    FROM BHMLogs AS L 
    ...
    WHERE D.TermYear = #year# AND D.Term = #term# AND L.StudentID = #RecNo# AND C.CategoryType = 'Reward'
</cfquery>

<cfquery name="getDetent" datasource="#dsn#">
    SELECT COUNT(L.LogID) AS Detention
    FROM BHMLogs AS L 
    INNER JOIN TermDates D ON D.StartDate <= L.LogDate AND D.EndDate >= L.LogDate 
    INNER JOIN BHMTypes T ON T.TypeID = L.TypeID 
    INNER JOIN BHMCategory C ON C.CategoryID = T.CategoryID
    WHERE D.TermYear = #year# AND D.Term = #term# AND L.StudentID = #RecNo# AND C.Category LIKE '%detention%' AND C.CategoryType = 'Consequence'
</cfquery>

<cfif ListFind('Yr10_SKABS,Yr11_SKABS,Yr12_SKABS', getList.AcYear[i])>
<tr>
<td>
<table cellpadding="0" cellspacing="0" width="100%" align="center">
<tr align="center" class="txtRep10">
<td width="22%" class="ball" style="background-color:##818285;color:##FFFFFF;"><b>House Points</b></td>
<td width="10%" class="btop bright bbottom"><cfif getMerit.Points NEQ "">#NumberFormat(getMerit.Points)#<cfelse>0</cfif></td>
<td width="24%" class="btop bright bbottom" style="background-color:##818285;color:##FFFFFF;"><b>Days Absent</b></td>
<td width="10%" class="btop bright bbottom"><cfif getStudAtt.Absent NEQ "">#NumberFormat(getStudAtt.Absent)#<cfelse>0</cfif></td>
<td width="24%" class="btop bright bbottom" style="background-color:##818285;color:##FFFFFF;"><b>Detentions</b></td>
<td width="10%" class="btop bright bbottom"><cfif getDetent.Detention NEQ "">#NumberFormat(getDetent.Detention)#<cfelse>0</cfif></td>
</tr>
</table>
</td>
<td></td>
...
<td width="5%">#Grade#</td>
<td width="95%">#ListFirst(GradeDesc,'(')# (#MinPercent#-#MaxPercent#%)</td>
</tr>
</cfloop>
</table>
</td>
...
<cfif ListFind('Yr7_SKABS,Yr8_SKABS,Yr9_SKABS', getList.AcYear[i])>
<tr>
<td style="height:10px;"></td>
</tr>
...
<td width="25%" style="padding:2px 0px;color:##FFFFFF;" class="ball">Attainment</td>
<td width="25%" style="color:##FFFFFF;" class="btop bright bbottom">Division</td>
<td width="25%" style="color:##FFFFFF;" class="btop bright bbottom">Grade Description</td>
<td width="25%" style="color:##FFFFFF;" class="btop bright bbottom">Overall Mark</td>
</tr>


<cfloop query="getGrades">
<tr align="center" class="txtRep10">
<td style="padding:2px 0px;" class="bleft bright bbottom">#MinPercent#-#MaxPercent#</td>
<td class="bright bbottom"><cfif Grade EQ "F">Fail<cfelse>Division #Grade#</cfif></td>
<td class="bright bbottom"><cfif Grade EQ "F">#ListFirst(GradeDet, " (")#<cfelse>#GradeDet#</cfif></td>
<td class="bright bbottom" <cfif Grade EQ "F">style="padding:2px 5px;background-color:##0061b0;color:##FFFFFF;" align="left"</cfif>><cfif Grade EQ "F"><b>#Replace(Replace(Replace(GradeDet, "Fail ", ""), "(", ""), ")", "")#</b><cfelse>#NumberFormat(Points)# - #NumberFormat(PointsAP)#</cfif></td>
</tr>
</cfloop>
</table>
</td>
...
<td class="btop bright bbottom">Teacher: <b>#ucFirst(threadResult.T1Title, true, true)#<cfif threadResult.T1Title NEQ "">.</cfif> #LEFT(threadResult.T1FName, 1)#<cfif threadResult.T1FName NEQ "">.</cfif> #ucFirst(threadResult.T1LName, true, true)#</b></td>
</tr>

<tr align="left" class="txtRep10" style="background-color:##818285;color:##FFFFFF;">
<td width="33%" class="bleft bright bbottom" style="padding:2px 5px;">Achievement: <cfif threadResult.getDet.ExamMark NEQ "">#threadResult.getDet.ExamGrade#</cfif></td>
<td width="34%" class="bright bbottom" style="padding:2px 5px;">Effort: #threadResult.getDet.Sumval#</td>
<td width="33%" class="bright bbottom" style="padding:2px 5px;">Exam: <cfif threadResult.getDet.ExamMark NEQ "">#NumberFormat(threadResult.getDet.ExamMark)#</cfif></td>
</tr>
...
<cfif getPage.PageNo EQ getSubj.Page>
<tr>
<td>
<table cellpadding="0" cellspacing="0" width="100%" align="center">
<tr>
...
<table cellpadding="0" cellspacing="0" width="100%" align="center">
<tr align="left" class="txtRep8">
<td width="33%" valign="bottom" style="padding-bottom:5px;">Page 1</td>
...
<cfloop index="k" from="2" to="#getPage.PageNo#">
...
<cfquery name="getSubj" datasource="#dsn#">
    ...
    LEFT JOIN ARCommSetup CS ON CS.TermYear = A.TermYear AND CS.Term = A.Term AND CS.StudYear = A.AcYear AND CS.Type = 2 AND CS.Reference = 'A'
    LEFT JOIN ARAcadComm SS ON SS.CommSetupID = CS.CommSetupID AND SS.AcadID = A.AcadID
    WHERE A.TermYear = #year# AND A.Term = #term# AND A.StudID = #RecNo# AND A.RepCard = 0 AND A.SType <> 'R' AND A.Page = #k#
    ORDER BY A.Seq
</cfquery>
...
<td>
<table cellpadding="0" cellspacing="0" width="100%" align="center">
<tr align="center" class="txtRep12Bold" valign="bottom">
<td width="15%" class="ball" style="background-color:##004fb0;color:##FFFFFF;padding:2px 0px;">Overall Mark</td>
<td width="85%"></td>
</tr>
<tr align="center" class="txtRep12Bold" valign="bottom">
<td class="bleft bright bbottom" style="padding:2px 0px;"><cfif getAverage.TotalMark NEQ "">#NumberFormat(getAverage.TotalMark)#</cfif></td>
<td></td>
</tr>
</table>
</td>
...
<table cellpadding="0" cellspacing="0" width="100%" align="center">
<tr align="left" class="txtRep8">
<td width="33%" valign="bottom" style="padding-bottom:5px;">Page #getSubj.Page#</td>
<td width="34%" align="center"><img src="#fileService.getFile(fileDir = "/keydoc/images", filename = "replogo.png", includeHTTP = "https:")#" style="height:1.5cm;"/></td>
<td width="33%">#getCF.RepData2#</td>
</tr>
</table>
