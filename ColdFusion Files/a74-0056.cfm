...
<cfquery name="getSubj" datasource="#dsn#">
    SELECT A.Seq, A.SCode, A.SubjName, A.Page, A.Section, A.Mark, A.Grade, A.T1Title, A.T1FName, A.T1LName
    FROM vARSubjAcads A
...
<td width="20%" style="padding:2px 0px;" rowspan="2" class="ball" valign="center">Subject</td>
<td width="23%" class="btop bright bbottom" rowspan="2" align="center">Teacher</td>
<td width="8%" class="btop bright bbottom" rowspan="2" valign="center">Cycle<br/>1</td>
<td width="8%" class="btop bright bbottom" rowspan="2" valign="center">Cycle<br/>2</td>
<td width="8%" class="btop bright bbottom" rowspan="2" valign="center">Cycle<br/>3</td>
<td colspan="3" class="btop bright bbottom">Current Cycle 4</td>
<td width="8%" class="btop bright bbottom" rowspan="2" valign="center">Year<br/>Mark</td>
</tr>
<tr align="center" class="txtRep11Bold" style="background-color:##b4c5e7;" valign="center">
<td width="8%" class="bright bbottom" valign="center">%</td>
<td width="8%" class="bright bbottom" valign="center">Effort</td>
<td width="9%" class="bright bbottom" align="center">Class <br/>Average</td>
</tr>
...
<cfquery name="THREAD.getDet" datasource="#ATTRIBUTES.dsn#">
	SELECT Nbr, CASE WHEN A.IsHeader = 1 THEN C.Mark ELSE A.Mark END AS Mark, 
                CASE WHEN A.IsHeader = 1 THEN C.Cycle1 ELSE ACDT.Mark END AS Cycle1, 
                CASE WHEN A.IsHeader = 1 THEN C.Cycle2 ELSE ACDE.Mark END AS Cycle2,
                CASE WHEN A.IsHeader = 1 THEN C.Cycle3 ELSE ACDS.Mark END AS Cycle3,
                CASE WHEN A.IsHeader = 1 THEN C.Cycle4 ELSE ACDF.Mark END AS Cycle4,
                CASE WHEN A.IsHeader = 1 THEN CM.AvgMark ELSE M.AvgMark END AS AvgMark,
                CASE WHEN A.IsHeader = 1 THEN C.YMMark ELSE A.YMMark END AS YMMark
	...
	FROM UTLNumbers N
        ...
        LEFT JOIN ARAssCol ACS ON ACS.ARSID = A.ARSID AND ACS.Lvl = 2 AND ACS.ColHeading LIKE '%C3%'
		LEFT JOIN ARAssColData ACDS ON ACDS.ARAssColID = ACS.ARAssColID AND ACDS.AcadID = A.AcadID

        LEFT JOIN ARAssCol ACF ON ACF.ARSID = A.ARSID AND ACF.Lvl = 2 AND ACF.ColHeading LIKE '%C4%'
		...
		LEFT JOIN (
			...
			ROUND(SUM(ROUND(ACDT.Mark, 0) * A.SubjWeight) / SUM(CASE WHEN ACDT.Mark IS NULL THEN NULL ELSE A.SubjWeight END), 0) AS Cycle1,
			ROUND(SUM(ROUND(ACDE.Mark, 0) * A.SubjWeight) / SUM(CASE WHEN ACDE.Mark IS NULL THEN NULL ELSE A.SubjWeight END), 0) AS Cycle2,
            ROUND(SUM(ROUND(ACDS.Mark, 0) * A.SubjWeight) / SUM(CASE WHEN ACDS.Mark IS NULL THEN NULL ELSE A.SubjWeight END), 0) AS Cycle3,
            ROUND(SUM(ROUND(ACDF.Mark, 0) * A.SubjWeight) / SUM(CASE WHEN ACDF.Mark IS NULL THEN NULL ELSE A.SubjWeight END), 0) AS Cycle4,
            ROUND(SUM(ROUND(A.YMMark, 0) * A.SubjWeight) / SUM(CASE WHEN A.YMMark IS NULL THEN NULL ELSE A.SubjWeight END), 0) AS YMMark
		FROM vARSubjAcads A
    ...
    LEFT JOIN (
        SELECT A.Term, Avg(ACDF.Mark) AS AvgMark
        FROM vARSubjAcads A
            LEFT JOIN ARAssCol ACF ON ACF.ARSID = A.ARSID AND ACF.Lvl = 2 AND ACF.ColHeading LIKE '%C4%'
		    LEFT JOIN ARAssColData ACDF ON ACDF.ARAssColID = ACF.ARAssColID AND ACDF.AcadID = A.AcadID
        WHERE A.TermYear = #ATTRIBUTES.tData.year# AND A.Term <= #ATTRIBUTES.tData.term# AND A.AcYear = '#ATTRIBUTES.tData.AcYear#' AND <cfif ATTRIBUTES.tData.Section NEQ "">A.Section = '#ATTRIBUTES.tData.Section#'<cfelse>A.Class = '#ATTRIBUTES.tData.Class#'</cfif> AND A.SCode = N'#ATTRIBUTES.tData.SCode#'
        GROUP BY A.Term
	) M ON M.Term = N.Nbr
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            ROUND(SUM(ROUND(ACDF.Mark, 2) * A.SubjWeight) / SUM(CASE WHEN ACDF.Mark IS NULL THEN NULL ELSE A.SubjWeight END), 2) AS Mark
            FROM vARSubjAcads A
                LEFT JOIN ARAssCol ACF ON ACF.ARSID = A.ARSID AND ACF.Lvl = 2 AND ACF.ColHeading LIKE '%C4%'
		        LEFT JOIN ARAssColData ACDF ON ACDF.ARAssColID = ACF.ARAssColID AND ACDF.AcadID = A.AcadID
            WHERE A.TermYear = <cfqueryparam cfsqltype="cf_sql_smallint" value="#ATTRIBUTES.tData.year#">
                AND A.Term <= <cfqueryparam cfsqltype="cf_sql_smallint" value="#ATTRIBUTES.tData.term#">
                AND A.AcYear = '#ATTRIBUTES.tData.AcYear#'
                AND <cfif ATTRIBUTES.tData.Section NEQ "">A.Section = '#ATTRIBUTES.tData.Section#'<cfelse>A.Class = '#ATTRIBUTES.tData.Class#'</cfif>
                AND A.PSSID IS NOT NULL
...
<td class="bright bbottom"><cfif threadResult.getDet.Cycle1[1] NEQ "">#NumberFormat(threadResult.getDet.Cycle1[1])#</cfif></td>
<td class="bright bbottom"><cfif threadResult.getDet.Cycle2[2] NEQ "">#NumberFormat(threadResult.getDet.Cycle2[2])#</cfif></td>
<td class="bright bbottom"><cfif threadResult.getDet.Cycle3[3] NEQ "">#NumberFormat(threadResult.getDet.Cycle3[3])#</cfif></td>
<td class="bright bbottom"><cfif threadResult.getDet.Cycle4[3] NEQ "">#NumberFormat(threadResult.getDet.Cycle4[3])#</cfif></td>
<td class="bright bbottom">#threadResult.getSubjSum.Sumval#</td>
<td class="bright bbottom"><cfif threadResult.getDet.AvgMark[3] NEQ "">#NumberFormat(threadResult.getDet.AvgMark[3])#</cfif></td>
<td class="bright bbottom"><cfif threadResult.getDet.YMMark[3] NEQ "">#NumberFormat(threadResult.getDet.YMMark[3])#</cfif></td>
</tr>
</cfif>
...
<cfquery name="getSubj" datasource="#dsn#">
    ...
    FROM vARSubjAcads A
        ...
        LEFT JOIN ARAcadComm SS ON SS.CommSetupID = CS.CommSetupID AND SS.AcadID = A.AcadID
    WHERE A.TermYear = #year# AND A.Term = #term# AND A.StudID = #RecNo# AND A.RepCard = 0 AND A.SType <> 'R' AND A.Page > 0 AND SS.Comment <> ''
    ORDER BY A.Seq
</cfquery>
...
<td colspan="2" class="bleft bright bbottom" style="padding:2px 5px;height:20px;">#Replace(threadResult.Comment, Chr(13), "<br />", "All")#</td>
</tr>


<cfif threadResult.CurrentRow NEQ threadResult.RecordCount>
<tr>
...
<tr align="left" class="txtRep10" valign="top">
<td colspan="2" class="bleft bright bbottom" style="padding:2px 5px;height:2cm;">#Replace(getComms2.Comment, Chr(13), "<br />", "All")#</td>
</tr>
</table>
</td>
</tr>
...
<tr align="left" class="txtRep10" valign="top">
<td colspan="2" class="bleft bright bbottom" style="padding:2px 5px;height:2cm;">#Replace(getTutorComms.Comment, Chr(13), "<br />", "All")#</td>
</tr>
</table>
</td>
</tr>
